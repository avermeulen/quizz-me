...
module.exports = function(models) {
    ...
    var resetQuiz = async function(req, res, next){
        try{
            const quiz_id = req.params.quiz_id;
            const group_id = req.params.group_id;

            var quiz = await findQuizById(quiz_id);
            quiz.answers = [];
            quiz.status = 'active';
            quiz.score = 0;
    ...
    var showQuizQuestion = async function(req, res, next) {

        try{
            var quiz_id = req.params.quiz_id,
            question_nr = Number(req.params.question_nr);
            let quiz = await findQuizById(quiz_id);
    ...
    var answerQuiz = async function(req, res, next) {
        try{
            const quiz_id = req.params.quiz_id;
    ...
    var answerQuizQuestion = async function(req, res, next) {
        ...
        try{
            ...
            if (lastQuestion) {
                await processQuizAnswers(quiz_id)
    ...
    var completed = async function(req, res, next) {
    ...
    var processQuizAnswers = async function(quiz_id) {
        var quiz = await findQuizById(quiz_id);
        ...
        var score:number = Math.ceil((totalCorrect / numberOfMcqs) * 100) ;
    ...
    var profile = async function(req, res, next) {
        try{
            const userQuizData = await services
    ...
    var showQuizzAllocationScreen = async function(req, res, next) {
    ...
    var cancel = async function(req, res, next){
        try{
            ...
            const user = await User.findById(ObjectId(quiz._user));
