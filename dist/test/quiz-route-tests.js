const assert = require('assert'), quizAnswerData = require('./data/quiz-answer-data'), courseData = require('./data/course-instance'), userData = require('./data/user.json'), quizLastAnswerData = require('./data/quiz-result-answer-last-question'), Promise = require('bluebird'), co = require('co'), mongoose = require('mongoose'), ObjectId = mongoose.Types.ObjectId, EmailQueue = require('../utilities/email-queue'), models = models = require('../models'), mongooseConnect = require('./mongoose-connect'), QuizRoutes = require('../routes/quiz-routes');
require('co-mocha');
            ...
            var res = {
                render: function (view, context) {
                    try {
                        ...
                        assert.equal(context.progress_message, expectedContext.progress_message);
                        done();
                    }
                    catch (err) {
                        done(err);
