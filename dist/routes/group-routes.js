const mongoose = require('mongoose'), _ = require('lodash'), ObjectId = mongoose.Types.ObjectId, render = require('../utilities/render'), reportErrors = require('../utilities/http_utilities').reportErrors, AllocateQuiz = require('../utilities/allocate-quiz'), coify = require('../utilities/coify');
module.exports = function (models) {
    const UserGroup = models.UserGroup, User = models.User, Course = models.Course, Quiz = models.Questionairre, allocateQuiz = AllocateQuiz(models);
    ...
    var showUserGroup = function (req, res, next) {
        return function* () {
            try {
                const group_id = ObjectId(req.params.group_id), userGroupData = yield findUsersInGroup(group_id), userGroup = userGroupData.userGroup, users = userGroupData.users, quizzes = yield Quiz
                    .find({
                    '_id': {
    ...
    var addGroup = function (req, res, next) {
        return function* () {
            ...
            var data = req.body, userIds = data.userId;
    ...
    const allocateQuizAction = function (req, res, next) {
        return function* () {
            try {
                const course_id = req.body.course_id, group_id = req.params.group_id, userGroupData = yield findUsersInGroup(group_id);
    ...
    const addUsers = function (req, res, next) {
        return function* () {
            try {
                ...
                userIds.forEach((uid) => userGroup.members.push(ObjectId(uid)));
                yield userGroup.save();
